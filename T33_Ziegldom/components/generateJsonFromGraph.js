...
function generateComposite(element) {
    const composite = {
        ...
        prompt: element.get('type') === 'standard.Circle' ? element.attr('prompt') : element.attr('label/text'),
        starter: element.attr('starter'),
        ...(element.get('type') === 'standard.Circle' && element.attr('isStarting') ? {
            isStarting: true
        } : {}),
        transitions: [],
    };

    const outgoingLinks = graph.getConnectedLinks(element, {
        outbound: true
    });
